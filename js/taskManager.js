...
createTaskHtml = (id, name, description, assignedTo, dueDate, status) => {
  let renderHtml = `
             <div class="col-4">
              <div class="card list-group-item" style="width:400px" id="mycard" data-task-id=${id}>
              ...
              <div class="card-body">
                ...
                <p>${description}</p>
              ${
    (status === 'Done') ? `<button type="button" class="btn btn-primary done-button invisible" > Mark as Done</button>` : `<button type="button" class="btn btn-primary done-button visible" > Mark as Done</button>`
    }
...
class TaskManager {
  ...
  render() {
    ...
    for (let i = 0; i < this.tasks.length; i++) {
      ...
      const taskHtml = createTaskHtml(this.tasks[i].id, this.tasks[i].name, this.tasks[i].description, this.tasks[i].assignedTo, this.tasks[i].dueDate, this.tasks[i].status);
